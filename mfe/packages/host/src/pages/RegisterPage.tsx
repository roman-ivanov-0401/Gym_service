...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(d?.error?.message ?? d?.message ?? 'Не удалось зарегистрироваться');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-zinc-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-card-lg p-8 w-full max-w-md ring-1 ring-zinc-200/80">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {([['Имя','text','name','Ваше имя'],['Электронная почта','email','email','you@example.com'],['Пароль','password','password','••••••••']] as [string,string,string,string][]).map(([label,type,key,ph]) => (
            <div key={key}>
        ...
        <p className="text-center text-sm text-zinc-500 mt-6">Уже есть аккаунт? <Link to="/login" className="text-teal-700 font-medium hover:text-teal-900 hover:underline">Войти</Link></p>
      </div>
    </div>
  );
}
